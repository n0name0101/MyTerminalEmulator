...
static inline void get_line_offset(Console *console, GtkTextIter *iter, gint y, gint x) {
    ...
    if (gtk_text_iter_ends_line(&line_end) == FALSE) {
        ...
        line_length = gtk_text_iter_get_offset(&line_end) - gtk_text_iter_get_offset(&line_start);
    }
...
static inline void get_line(Console *console, GtkTextIter *iter, gint y) {
    // Pastikan buffer memiliki setidaknya y + 1 baris.
    gint line_count = gtk_text_buffer_get_line_count(console->buffer);
    if (y >= line_count) {
        GtkTextIter end_iter;
    ...
    gtk_text_buffer_get_iter_at_line(console->buffer, iter, y);
}
